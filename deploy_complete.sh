...
# Script de despliegue completo para KidsFun Backend
...
if [ ! -f ".env" ]; then
    ...
    print_warning "⚠️  IMPORTANTE: Edita el archivo .env con las configuraciones correctas"
    print_warning "   Especialmente DATABASE_URL, SECRET_KEY y credenciales de email"
    echo ""
...
sudo -u kidsfun bash -c "source .env/bin/activate && pip install --upgrade pip"
...
# 10. Verificar conexión a base de datos
print_status "Verificando conexión a base de datos..."
if sudo -u kidsfun bash -c "source .env/bin/activate && python -c \"
import sys
sys.path.append('.')
...
if sudo -u kidsfun bash -c "source .env/bin/activate && alembic upgrade head"; then
...
# 19. Crear script de actualización
